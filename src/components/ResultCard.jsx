...
import { Info, ArrowLeft, ExternalLink, ShieldAlert, BrainCircuit } from 'lucide-react'
...
export default function ResultCard({ entry, onReset, currency = '৳' }) {
  ...
  return (
    // Max-w-2xl ensures the card never stretches too wide, keeping text highly readable
    <div className="max-w-2xl mx-auto w-full animate-fade-up">
      ...
      <div className={clsx('card p-0 overflow-hidden shadow-2xl border', theme.border)}>
        ...
        {/* Section 2: Wali's Counsel (The core psychological friction) */}
        <div className="p-6 md:p-8 bg-zinc-900/40 relative">
          {/* Subtle background quotation mark for aesthetic depth */}
          ...
          <p className="text-lg md:text-xl text-zinc-300 font-display italic leading-relaxed relative z-10">
            "{entry.argument || 'Reflect on this purchase. Is it a true necessity or a fleeting desire driven by the Nafs?'}"
          </p>

          {/* Flags are moved here, contextualizing the AI's argument */}
        ...
        <div className="p-6 md:p-8 bg-zinc-950/50 flex flex-col md:flex-row md:items-center justify-between gap-6 border-y border-zinc-800/60 shadow-inner">
          <div>
            ...
            <p className="text-sm text-zinc-400 font-medium">If redirected into <span className="text-zinc-200">{entry.investment_vehicle}</span></p>
          </div>
